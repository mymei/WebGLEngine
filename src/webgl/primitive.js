...
Node.prototype.applyMatrix = function(matrix) {
	var sx = vec3.set(this.scale, matrix[0], matrix[1], matrix[2]).length();
	var sy = vec3.set(this.scale, matrix[4], matrix[5], matrix[6]).length();
	var sz = vec3.set(this.scale, matrix[8], matrix[9], matrix[10]).length();
...
Node.prototype.draw = function(camera_trans, time) {

	if (this.de) {
		this.de.scene.applyAnimation(this.de.asset.animation, time);
		this.de.draw(gl, camera_trans, this.matrixWorld);
	}

	for (var i = 0; i < this.children.length; i ++) {
		this.children[i].draw(camera_trans, time);
	}
}